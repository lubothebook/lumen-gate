...
template GateVm() {
    ...
    for (var t = 0; t < T; t++) {
        ...
        for (var k = 0; k < K; k++) {
            ...
            if (k == 0) {
                cellAcc[t][k] <== lineSel[t][k].out * program[k];
            } else {
                cellAcc[t][k] <== cellAcc[t][k - 1] + lineSel[t][k].out * program[k];
            }
        }
        ...
        for (var j = 0; j < R; j++) {
            ...
            eqC[t][j].in <== c[t] - j;
            if (j == 0) {
                vaAcc[t][j] <== eqA[t][j].out * regs[t][j];
                vbAcc[t][j] <== eqB[t][j].out * regs[t][j];
                vtAcc[t][j] <== eqB[t][j].out * j;
            } else {
                vaAcc[t][j] <== vaAcc[t][j - 1] + eqA[t][j].out * regs[t][j];
                vbAcc[t][j] <== vbAcc[t][j - 1] + eqB[t][j].out * regs[t][j];
        ...
        for (var j = 0; j < R; j++) {
            ...
            nextVal[t][j] <== regs[t][j] + gate[t][j] * (result[t] - regs[t][j]);
            if (t < T - 1) {
                nextVal[t][j] === regs[t + 1][j];
            }
        }
        // The public end root is the derived final r2 — the row the prover
        // declared is never trusted: end_root === (what the last step
        // computes). A prover lying in regs about the output has no witness.
        end_root === nextVal[T - 1][2];

        // ---- halt: sticky, and set only by a HALT line -----------------------
        haltSet[t] <== selHalt[t] * active[t];
        haltedNext[t] <== halted[t] + haltSet[t] - haltSet[t] * halted[t];
        if (t < T - 1) {
        ...
        advance[t] <== (1 - jnzTaken[t]) * (pc[t] + 1) + jnzTaken[t] * targetLine[t];
        pcNext[t] <== haltedNext[t] * pc[t] + (1 - haltedNext[t]) * advance[t];
        if (t < T - 1) {
            pcNext[t] === pc[t + 1];
        }
        ...
        hashCount[t + 1] <== hashCount[t] + selPose[t] * active[t];
    }
