...
template GateVm(K, T, R) {
    // pc must address every line and nothing beyond: the decomposition width
    // is log2(K), computed rather than hardcoded, because the 32-line
    // sibling of this circuit would otherwise inherit an 8-line bound and
    // silently accept program counters the fold cannot even commit to.
    var PC_BITS = 0;
    var kp = K;
    while (kp > 1) {
        kp >>= 1;
