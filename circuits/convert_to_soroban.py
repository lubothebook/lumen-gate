...
Usage:
    python3 convert_to_soroban.py vk.json proof.json public.json out_prefix
        [--public-names a,b,c,...] [--rust-rust-file path] [--expect-inputs N]
...
def main():
    ...
    if rust_out:
        write_rust_vectors(rust_out, vk, vb, pb, inputs, names)


def write_rust_vectors(path, vk, vb, pb, inputs, names):
