...
use p3_air::symbolic::{get_max_constraint_degree_extension, AirLayout, SymbolicAirBuilder};
use p3_air::Air;
...
mod tests {
    ...
    #[derive(Debug)]
    struct MockAir {
        constraints: Vec<SymbolicVariable<Goldilocks>>,
        width: usize,
    }

    impl BaseAir<Goldilocks> for MockAir {
        fn width(&self) -> usize {
            self.width
        }
    }

    impl Air<SymbolicAirBuilder<Goldilocks>> for MockAir {
        fn eval(&self, builder: &mut SymbolicAirBuilder<Goldilocks>) {
    ...
    fn air_layout(air: &impl BaseAir<Goldilocks>, preprocessed_width: usize) -> AirLayout {
        AirLayout {
            preprocessed_width,
    ...
    impl BaseAir<Goldilocks> for HintedMockAir {
        fn width(&self) -> usize {
            self.width
        }
    ...
    impl Air<SymbolicAirBuilder<Goldilocks>> for HintedMockAir {
        fn eval(&self, builder: &mut SymbolicAirBuilder<Goldilocks>) {
            for constraint in &self.constraints {
                builder.assert_zero(*constraint);
            }
