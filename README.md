...
- **The wall is a backdrop, not a carpet.** One element per *pitch*, not one per pixel of tile: the tile keeps its 60px and its one-asset-pixel-per-screen-pixel rule, and the pitch opens up with the screen — one tile on a phone, two on a laptop, four on a wide display. A 1920×1080 screen carries **40 cubes instead of 576**, which is what a projector gets asked to repaint on every pointer move, and `tools/check-grid-fx.js` asserts the arithmetic at three widths and two device pixel ratios.
