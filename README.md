...
- **The Groth16 lane proves a quorum and binds the three roots; it is not a signature verifier.** The circuit header says so in its own words. It establishes that enough approvals exist over a bitmap and that `prev_state_root`, `state_root` and `event_root` participate in one Poseidon relation, so none of them is decorative metadata. It does **not** establish that any particular validator signed anything. A production ZK lane replaces the bitmap with a real signature gadget — a step whose constraint cost is measured (§5e of [`docs/PROVING_SYSTEM.md`](docs/PROVING_SYSTEM.md): 8,086 for a full EdDSA verifier), not a step whose cost is feared.
...
- **The console is wired to the live contracts and is capability-gated.** Addresses come from the deployment manifest through `/api/status`, finality comes from a live contract simulation, balances come from Horizon, and the audit table comes from the record the loop writes. A fourth surface — the receipts card — fetches nothing at all: it renders from a module generated at build time from `deployments/*.json`, and where a receipt is silent it shows an em-dash and says so rather than inventing a zero. Controls that this particular deployment cannot honour are disabled with the reason shown — the relay button when no operator is configured, the lock button when no source adapter exists. The honest limits: the hosted console is a verification and operator surface, not a customer-facing anchor; the source chain is reachable only if someone exposes the simulator; and the outbound burn still needs the user's own Freighter signature and fee.
...
Five Groth16 circuits exist today as lanes (the fifth is the 32-line sibling of the gate-vm machine), plus two feasibility probes that compile but never prove; all lanes are compiled ahead of time and verified on-chain through Stellar's native BN254 host functions:
...
| [`circuits/gate_vm32.circom`](circuits/gate_vm32.circom) | **22861 non-linear / 21779 linear** | the gate-vm statement at quadrupled scale: one core (`gate_vm_core.circom`), instantiated twice, so the growth rate is the auditable fact — and the padded-commit property (same execution output, different program commitment) is pinned by `cargo test -p gate_vm` rather than asserted |
...
**The design is described by the page itself, in the About section.** A panel there makes the three claims this section makes, and renders each one from the interface as it loads rather than from prose or from a screenshot: it counts and measures the strips, it hands you six real cubes painted from the submitted 60×60 tile so you can put the pointer on one and watch the 4-pixel ring, and it lists every disabled control with its reason by reading the buttons and the notes they point at out of the DOM. Images were tried first and are the wrong answer — a committed screenshot is a claim that goes stale the moment the layout moves, and this project does not ship claims it cannot keep. `tools/check-live-page.js` and `tools/check-live-actions.js` assert the panel's numbers against the page's own measurements, so the panel cannot describe a page that is not there.
...
- **The grid reacts to the pointer.** Moving across it snaps a cell-sized frame to the cube under the cursor, with a **4 screen pixel** inner border (`--ring: 4px / devicePixelRatio`), aligned to the same lattice, so what lights up is exactly one block. It is painting only, never a control, and it disappears where there is nothing to point at — over the black strips, the cards, the wallet band, the header and the footer. This bullet was wrong for three rounds, and the way it was wrong is worth keeping: the frame was written as a `:hover` state on the cube, and the lattice is painted at `z-index: -1`, so every wrapper above it — section, shell, body — won the browser's hit test and the frame never once appeared on the live page. A rule in a stylesheet, a sentence here, a green check, and no frame. It is painted by pointer tracking now (`initLatticeFrame()` in `frontend/src/app.js`), which knows which surfaces cover the lattice, and the claim is checked twice: `tools/check-grid-fx.js` drives the hit-test logic with synthetic stacks, and `tools/check-live-page.js` drives the real page in a real browser.
- **The grid only stops where the text is.** Text does not sit on a painted section: each row of it carries its own **full-bleed black strip**, infinite sideways, and the lattice stays visible in the gap between one strip and the next — which is where the pointer frame gets to play. An earlier round read "black strips" as one block per section, which painted over the artwork this page exists to show; the strips are per row now, and the ink is padded back to the shell measure so a row in one strip still lines up with the rows above and below it. The band **hugs the row it carries** — 18px of padding inside it at desktop width, with the air between rows coming from the row gap where the lattice shows — and the hero is the one area with no band at all. `tools/check-live-page.js` measures all eighteen rows: every strip runs edge to edge, every pair leaves an open gap, no band pads more than 24px, nothing pushes the page into a horizontal scrollbar, and the wallet's top edge lands inside the first screen.

- **One ring, and it follows the pointer anywhere.** The wall's blocks sit edge to edge — the chain is contiguous — and the ring that marks the block under the pointer is a single overlay element, positioned by arithmetic on the pointer's own coordinates (`floor(x / cell)`, `floor(y / cell)`). It used to ask the document what was under the cursor and give up when the answer was not a cube, which meant it vanished over every card, every strip and the header; now it does not care what is painted above it, so a card, a strip and open lattice all frame the same way. It is drawn above the page and below the dock, and it takes no clicks. The first screen carries a row of the same blocks at the tile's own 60px, so the source chain is visible before any text is read; on a narrow display one block is shown on its own, at its own size rather than a resized copy.
...
Two habits keep the console honest about its own failure modes. The first is that **a control that cannot act says why, twice**: in the note under it and in its own title, with the title copied from the note by one table (`DISABLED_REASONS`) rather than written out again, because two hand-written copies of one reason drift and the one a reader hovers is then the wrong one. The second is that the page is **clicked, not read**: [`tools/check-live-actions.js`](tools/check-live-actions.js) opens the real page in a real browser and clicks every control, asserting an observable consequence for each - a log line, a note that changed, a pane that switched, a dialog that opened, a table that refreshed - and holds every disabled control to its stated reason. Its first run found four disabled controls and two of them silent; that is where `DISABLED_REASONS` came from. Both browser harnesses take a URL, so they run against the live deployment as well as against a local server, and a check that only ever ran on a developer's machine would not have caught the serverless-only failures this project has already had once.

It is not in CI on purpose: it needs a browser and a running dev server, and a check that cannot run must say so rather than pass by default. Neither is it allowed to guess when the page is ready. A click that lands before the boot sequence has run does nothing at all, which looks exactly like a dead button - the first production run of this check failed for that reason alone, and reporting a dead button that is not dead is worse than reporting nothing. The page sets a boot beacon when it has finished wiring, the harness accepts the beacon or, for builds that predate it, the network pill leaving its "connecting" state.
...
Amounts are typed the way a wallet shows them — `13.7`, not `137000000` — and the base-unit integer is stated underneath (`13.7` → `137,000,000 base units on the wire`) rather than hidden, because that integer is what goes to the contract. The conversion is string arithmetic on purpose: `13.7 * 1e7` is `136999999.99` in binary floating point, and a bridge that sends that has a bug that only appears on some amounts. An amount with an eighth decimal place, a zero, or a negative is refused with a reason.

Buttons carry three weights so the demo path is never confused with the operator shelf: `primary` is the next step of the demo (lock, burn, open the withdrawal, settle), a plain button is a real but optional action, and `.mini` is housekeeping — copy a command, set a token, fill a field. Before this the page had two weights and a judge could wander off the path by clicking *Copy the command instead*. The card header states whether this tab may write, and setting the operator token repaints that state immediately rather than waiting for the next status poll.
...
- **Signatures inside the circuit.** Both Groth16 lanes prove a quorum of approval *bits*; neither verifies a signature. The chained lane carries that quorum step by step and binds each step's evidence into the state it produces, and it persists nothing that settlement anchors on. Putting a real signature gadget inside the circuit is the next step, not a relabeling of this one — and its budget is now measured rather than promised: a complete EdDSA-over-babyjubjub verifier costs 8,086 constraints on the pinned stack ([`circuits/signature_gadget_probe.circom`](circuits/signature_gadget_probe.circom), [PROVING_SYSTEM §5e](docs/PROVING_SYSTEM.md)), so affordability is off the list of objections; what the list still carries is a pairing library for BLS and the source chain's own hash-to-curve path, neither of which ships in this repository's pinned circomlib cut.
