...
| `POST /v1/sep10/auth` | verification of the client's signature over that exact transaction, using the SDK's own SEP-10 reader. For an on-ledger account the signer record and medium threshold are read from Horizon and the collected weight must meet it (multisig accounts authenticate with multisig weight); for an account that is not on the ledger yet the master key is verified, the case the spec reserves for it. The method used is returned in the response, then a short-lived HS256 JWT |
